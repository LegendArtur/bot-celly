# Celly v1 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build `Celly`, a Discord bot that runs one supervised `opencode serve` per `sbx` sandbox and maps each Discord channel to a project and each thread to an OpenCode session.
...
**Spec:** `docs/superpowers/specs/2026-09-25-celly-v1-design.md`
...
- Sandbox name: `celly-<slug>`, `<= 63` chars, `[a-z0-9.-]`, no trailing `-`/`.`, unique vs DB and `sbx ls --json`.
...
git commit -m "chore: scaffold Celly project"
```
...
test("handleCommand defers and answers status", async () => {
  ...
  db.projects.insertProvisioning({ channelId: "c", guildId: "g", name: "demo", directory: "C:\\p", sandboxPath: null, sandboxName: "celly-demo", hostPort: 4300, serverPassword: "pw", createdAt: 1 })
...
**Files:**
- Create: `src/lock.ts`, `src/index.ts`
- Test: `test/lock.test.ts`

**Interfaces:**
- Consumes: everything above.
- Produces: `acquireLock(port: number): Promise<{ release(): void }>` from `src/lock.ts` (loopback listener; rejects if held).
...
- [ ] **Step 3: Implement `src/lock.ts` and `src/index.ts`**
...
async function main() {
  ...
  const projects = new ProjectService({
    ...
    createChannel: async (name) => {
      const cat = cfg.categoryId ?? (await guild.channels.create({ name: "Forge", type: ChannelType.GuildCategory })).id
      const ch = await guild.channels.create({ name, parent: cat, type: ChannelType.GuildText })
      return ch.id
    },
...
Unused imports (`PermissionsBitField`) are removed during implementation. The message/interaction handlers and the EventRouter wiring described in Tasks 9-15 are connected here; `main()` runs only when this file is executed, so tests import `src/lock.ts` rather than `src/index.ts`.

- [ ] **Step 4: Run lock test, typecheck, commit** — `npx vitest run test/lock.test.ts` PASS; `npm run typecheck` passes. Then:

```bash
git add src/index.ts src/lock.ts test/lock.test.ts
