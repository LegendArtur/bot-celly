...
import { BOOTSTRAP_SCRIPT, BOOTSTRAP_VERIFY, buildCelyConfigJson, buildOpencodeEnv } from "../src/opencode.ts"

const STAGED_JSON = "/tmp/cely-opencode.json"
const STAGED_ENV = "/tmp/cely-opencode.env"
...
test.skipIf(onWindows)("bootstrap script installs the cely config and a 0600 env file, idempotently", () => {
  const home = mkdtempSync(join(tmpdir(), "cely-home-"))
  const stageDir = mkdtempSync(join(tmpdir(), "cely-stage-"))
  const json = join(stageDir, "opencode.json")
  const env = join(stageDir, "opencode.env")
  try {
    writeFileSync(json, buildCelyConfigJson(), { mode: 0o600 })
    ...
    const installedJson = join(home, ".config", "cely", "opencode.json")
    const installedEnv = join(home, ".config", "cely", "opencode.env")
    ...
    rmSync(join(home, ".config", "cely"), { recursive: true, force: true })
